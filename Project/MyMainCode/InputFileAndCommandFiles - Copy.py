...
class Barrier():
    ...
    def __init__(self,path,vehicleType,boundary,barrierLength,angle,writeFile):
        self.path = path
        self.writeFile = writeFile
        self.barrierLength = barrierLength
        self.angle = angle
    ...
    def write(self):
        ...
        if self.angle == 90:
            ...
            line5 = 'TRANSL '+str(math.ceil((-1500*num.cos(num.pi*self.angle/180))))+','+str(math.ceil((-1500*num.sin(num.pi*self.angle/180)))-0.5*self.truckWidth)+', 0'
        line6 = "*INCLUDE_TRANSFORM"
        line7 = self.path+ '/1-Barriers/'+self.boundary+'/02JersyBarrier'+str(self.barrierLength)+'m.k'
        line8 = '0, 0, 0, 0, 0, 0, 0'
        line9 = '0,'
        line10 = '0, 0,0, 0, 0'
        line11= '3'
...
class Pier():
    ...
    def __init__(self,path,vehicleType, boundary, barrierLength, angle, distance, pierType, writeFile):
        self.path = path
        self.writeFile = writeFile
        self.barrierLength = barrierLength
        self.angle = angle
        ...
        phi=(math.atan(self.location/(self.distance-1500)))*(180/num.pi)
        hypo=(self.distance-1500)/math.cos(num.pi*phi/180)
...
class Create():
    def __init__(self):
        ...
        self.Nodes = [[4000080  , 4000302  , 4000303 ,  4000304 ,  4000305 ,  4000306 ,  4000307  , 4000308,
           ...
           4006818 ,  4006819 ,  4006820  , 4006821 ,  4006822  , 4006823   ,4006824],[4000080,   4000302,   4000303 ,  4000304 ,  4000305  , 4000306 ,  4000307,   4000308,
           4000309  , 4000310  , 4000311  , 4000312  , 4000313 ,  4000314  , 4000315  , 4000316,
           4000317  , 4000318  , 4000319  , 4000320  , 4000321 ,  4000322  , 4000323  , 4000324,
           4000325  , 4000326  , 4000327  , 4000328  , 4000329 ,  4000330  , 4000331  , 4000332,
           4000333  , 4000334  , 4000335  , 4000336  , 4000337  , 4000338  , 4000339  , 4000340,
        ...
        self.Accel = [2000145,2000368] 
        self.offset = [0,-0.3]

    def createPhase1InputFiles(self):
        root = Tk()
        root.withdraw()
            ...
            for j in range (len(self.parameters.barrierBoundary)):
                ...
                for k in range (len(self.parameters.barrierLength)):
                    ...
                    for l in range (len(self.parameters.anglesOfAttack)):
                        ...
                        vehicle.write()
                        barrier = Barrier(dataPath,i,j,self.parameters.barrierLength[k],self.parameters.anglesOfAttack[l],lsdynaInputFile)
                        barrier.write()
                ...
                for k in range (1,len(self.parameters.barrierLength)):
                    ...
                    for l in range (len(self.parameters.anglesOfAttack)):
                        ...
                        for m in range (len(self.parameters.pierLocation[j])):
                            ...
                            barrier = Barrier(dataPath,i,j,self.parameters.barrierLength[k],self.parameters.anglesOfAttack[l],lsdynaInputFile)
                            barrier.write()
                            pier = Pier(dataPath,i,j,self.parameters.barrierLength[k],self.parameters.anglesOfAttack[l],self.parameters.pierLocation[j][m],1,lsdynaInputFile)
                            pier.write()
                            control = Control(dataPath,i,lsdynaInputFile)
                            control.write()
                            lsdynaInputFile. close()
            ...
            for j in range (len(self.parameters.barrierBoundary)):
                ...
                for k in range (1,len(self.parameters.barrierLength)):
                    ...
                    for l in range (len(self.parameters.anglesOfAttack)):
                        cwd_l = os.path.join(cwd_k,str(self.parameters.anglesOfAttack[l]))
                        for m in range (1):
                            cwd_m = os.path.join(cwd_l,str(self.parameters.pierLocationCaseStudy[counter]))
                            ext=".k"
                            Name = self.parameters.vehicleType[i]+'_'+self.parameters.barrierBoundary[j]+'_'+str(self.parameters.barrierLength[k])+'_'+str(self.parameters.anglesOfAttack[l])+'_'+str(self.parameters.pierLocationCaseStudy[counter])
                            lsdynaInputFilePath = os.path.join(cwd_m,Name+ext)
                            lsdynaInputFile = open(lsdynaInputFilePath,'w')
                            lsdynaInputFile.write('*KEYWORD'+'\n')
                            vehicle = Vehicle(dataPath,i,lsdynaInputFile)
                            ...
                            barrier = Barrier(dataPath,i,j,self.parameters.barrierLength[k],self.parameters.anglesOfAttack[l],lsdynaInputFile)
                            barrier.write()
                            pier = Pier(dataPath,i,j,self.parameters.barrierLength[k],self.parameters.anglesOfAttack[l],self.parameters.pierLocationCaseStudy[counter],2,lsdynaInputFile)
                            pier.write()
                            control = Control(dataPath,i,lsdynaInputFile)
                            control.write()
                            lsdynaInputFile. close() 
    ...
    def createLsdynaPhase1CommandFile(self):
        root = Tk()
        root.withdraw()
    ...
    def createLsPrePostPhase1CommandFile(self):
        root = Tk()
        root.withdraw()
            ...
            for j in range (len(self.parameters.barrierBoundary)):
                ...
                for k in range (len(self.parameters.barrierLength)):
                    ...
                    for l in range (len(self.parameters.anglesOfAttack)):
                        ...
                        line2='ascii rcforc plot 4 Sl-4000000'
                        ...
                        line2='ascii nodout plot 2 '
                        for node in range (len(self.Nodes[k])):
                            line2=line2+str(self.Nodes[k][node])+'/'
